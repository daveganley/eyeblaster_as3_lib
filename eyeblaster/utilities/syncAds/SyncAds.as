//****************************************************************************
//class  eyeblaster.utilities.syncAds.SyncAds
//-------------------------------------------
...
{
	import eyeblaster.utilities.syncAds.SyncSearchedAsset;
	import eyeblaster.utilities.syncAds.SyncLocalConnection;
	import eyeblaster.events.EBSyncEvent;
	...
	[Event(EBSyncEvent.CONNECTION_FOUND, type="eyeblater.events.EBSyncEvent")]
	[Event(EBSyncEvent.CONNECTION_NOT_FOUND, type="eyeblater.events.EBSyncEvent")]
	...
	public class SyncAds extends MovieClip
	{
		...
		// A string that holds the synchronization level group: values "Ad" or "Campaign"
		...
		// combination between the asset name, the sync level and unique number of the page the ads are displayed.
		// The connection need to be unique for each page since when there are some connections with the same name in other sessions the connection is not good.
		...
		public var compName:String = "SyncAds";	//The component name to be used for components detection.
		
		//++++++++++++++++++++++++++++++++++++++++++++++++++++
		//						Getter/Setter Methods
		//++++++++++++++++++++++++++++++++++++++++++++++++++++
		
		//sets the level the resource is using
		//Note: Setting the attribute is optional, but if used should be the first to use
		//	Parameters:
		//		Possible values:
		//			"ad" 
		//			"campaign"(default)
		public function set level(lvl:String):void
		{
			...
			if(lvl.toLowerCase() == "ad" && typeof(EBBase.urlParams.ebAdIdentifier) != "undefined")
			{
				this._strLevel = EBBase.urlParams.ebAdIdentifier;
				this._lvl = "Ad";
			}
			else if(lvl.toLowerCase() == "campaign" && typeof(EBBase.urlParams.ebCampaignID) != "undefined")
			{
				this._strLevel = EBBase.urlParams.ebCampaignID;
				this._lvl = "Campaign";
			}
			else
			{
		...
		public function openConnection(assetName:String):Boolean
		{
			try
			{
		        ...
		        //report to the js for the interaction monitor(cmd,param1|param2|..)
		        EBBase.handleCommand("ebSyncAdsInteraction", "'openConnection'," + assetName);
				...
				this._strConnName = _strResName + "_" + _strLevel + "_" + EBBase.urlParams.ebPageGUID;
		...
		public function findConnection(assetName:String,secTimeout:Number = -1):void
		{
			try
			{
		        ...
		        //report to the js for the interaction monitor(cmd,param1|param2|..)
		        EBBase.handleCommand("ebSyncAdsInteraction", "'findConnection'," + assetName + "|" + secTimeout);
		...
		public function callConnection(destAssetName:String, functionName:String, Params:String = ""):void
		{
			try
			{
		        ...
		        //report to the js for the interaction monitor(cmd,param1|param2|..)
		        EBBase.handleCommand("ebSyncAdsInteraction", "'callConnection'," + destAssetName + "|" + functionName + "|" + Params);

				//callConnection parameters
				var sendParams = arguments;
				
				//transfer destAssetName to the connection name by adding _strLevel 
				sendParams[0] += "_" + this._strLevel + "_" + EBBase.urlParams.ebPageGUID;
				...
				this._sendingLC.send(destAssetName + "_" + this._strLevel + "_" + EBBase.urlParams.ebPageGUID,"handleEvent",functionName,Params);
			}
			catch(error:Error)
			{
		...
		public function closeConnection():void
		{
			try
			{
		        ...
		        //report to the js for the interaction monitor(cmd,param1|param2|..)
		        EBBase.handleCommand("ebSyncAdsInteraction", "closeConnection");
		...
		public function assetFound(assetName:String):void
		{
			try
			{
				...
				if (this._arrFindAsset[index].isLookedFor)
				{
					...
					//report to the js for the interaction monitor(cmd,param1|param2|..)
					EBBase.handleCommand("ebSyncAdsInteraction" , "'onConnectionFound'," + assetName);
					Tracer.debugTrace("SyncAds: onConnectionFound event is triggered for asset "+assetName,2);
					//trigger event
					dispatchEvent(new EBSyncEvent(EBSyncEvent.CONNECTION_FOUND,assetName));
				}
				else
				{
		...
		private function _initAttr():void
		{
			try
			{
				...
				if(typeof(EBBase.urlParams.ebPageGUID) == "undefined")
				{
					EBBase.urlParams.ebPageGUID = "";
				}
		...
		private function _initComp():void
		{
			...
			EBBase.ebSetComponentName("SyncAds");
			...
			if(typeof(EBBase.urlParams.ebCampaignID) == "undefined" || typeof(EBBase.urlParams.ebAdIdentifier) == "undefined")
			{
		...
		private function _initFindLcConnection():void
		{
			// Creating name for the connection
			 this._findLcConnName = "connection" + int(Math.random()*10000) + "_" +this. _strLevel + "_" + EBBase.urlParams.ebPageGUID;
			try
			{
				...
				if (asset.isLookedFor)
				{
					if(asset.numOfInterval != -1)
					{
						...
						if(asset.numOfInterval == 0)
						{
						   ...
						   //report to the js for the interaction monitor(cmd,param1|param2|..)
							EBBase.handleCommand("ebSyncAdsInteraction", "'onConnectionNotFound'," + asset.assetName);
							Tracer.debugTrace("SyncAds: onConnectionNotFound event is triggered for asset "+asset.assetName,2);
							dispatchEvent(new EBSyncEvent(EBSyncEvent.CONNECTION_NOT_FOUND,asset.assetName));
						}
						// The number of calls didn't reach its limit and therefore,
						// we should try to call the areUThere function. 
						else 
						{
							this._sendingLC.send(asset.assetName + "_" + this._strLevel + "_" + EBBase.urlParams.ebPageGUID,functionName,Params);
						}
					}
					...
					{
						this._sendingLC.send(asset.assetName + "_" + _strLevel + "_" + EBBase.urlParams.ebPageGUID,functionName,Params);
					}
