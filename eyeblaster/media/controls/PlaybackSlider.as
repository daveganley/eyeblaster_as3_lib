//****************************************************************************
//class eyeblaster.media.controls.PlaybackSlider
//---------------------------------------
...
package eyeblaster.media.controls
{
	...
	[Event("ebVBPauseForSlider", type="mx.eyeblaster.General.VideoEvent")]
	[Event("ebVBPlayAfterPauseForSlider", type="mx.eyeblaster.General.VideoEvent")]
	[Event("ebVBSendSliderDraggedInteraction", type="mx.eyeblaster.General.VideoEvent")]
	public class PlaybackSlider extends BasePlayerCtrl
	{
		...
		private var _strVideoStatus:String=""; // the video status - Error, Idle, Loading, Buffering, Ready, Playing, Paused, Stopped, Ended
		
		private var _fDraggingPlaybackSlider; // Indicates whether dragging is on (default false).
		private var _draggingAreaWidth:Number; //this._trail.width - this._slider.width (gets value in _startDrag function)
		...
		public function PlaybackSlider()
		{
			try
			{
				...
				EBBase.ebSetComponentName("PlaybackSlider");
			}catch (error:Error)
			{
		...
		public override function handleEvent(evt:Object):void
		{
			try
			{
				...
				if (evt.hasOwnProperty("value"))
				{
					...
					switch (evt.type)
					{
						case "ebVLPlayProgress":
							if(evt.value==0)
								this._fIgnoreEvent=false;
							this._handlePlayProgress(evt.value);
							Tracer.debugTrace("PlaybackSlider: handle _handlePlayProgress event, value: " + evt.value, 4);
							break;
						case "ebVLLoadProgress":
							this._handleLoadProgress(evt);
							break;
						///Receive the ebVLStatusChanged event from VideoLoader in order to save the video status before start draging the slider.
						case "ebVLStatusChanged":
							if(!_fDraggingPlaybackSlider)
								this._strVideoStatus = evt.value;
							break;
						case "ebUpdatePlaybackSlider":
							removeEventListeners(); //remove dragging listeners in case user dragged the slider to the end of video
		...
		private function _enableDragging():void
		{
			try
			{
				...
				var loadBar = this._loadBar;		//loadBar (dragging area)
				...
				slider.addEventListener(MouseEvent.MOUSE_UP, _mouseUpDraggingEvent);
				//	update value
				trail.addEventListener(MouseEvent.CLICK, _updateUponClick);
				loadBar.addEventListener(MouseEvent.CLICK, _updateUponClick);
			}catch (error:Error)
			{
		...
		{
			Tracer.debugTrace("PlaybackSlider: _startDrag", 4);
			stage.addEventListener(MouseEvent.MOUSE_UP, _releaseOutsideDragging);
			this._slider.addEventListener(MouseEvent.MOUSE_MOVE, _updateUponDrag);
			//update the volume upon mouse move while the slider is dragged AND user moves the mouse out of the slider rollOver rect
			stage.addEventListener(MouseEvent.MOUSE_MOVE, _updateIfMouseMoveOutOfSlider);
			///block ebVLStatusChanged event
			_fDraggingPlaybackSlider = true;
			//fire pause event
			var ev:VideoEvent = new VideoEvent("ebVBPauseForSlider", "");
			dispatchEvent(ev);
			...
			_draggingAreaWidth = (this._trail.width - this._slider.width);
			//start drag
			this._slider.startDrag(false, new Rectangle(_x, _y, _draggingAreaWidth + 1, _y));
        }
		
		//================================
		//	function _mouseUpDraggingEvent
		//================================
		//This function triggers from the listener of the slider as result of Mouse_UP event
		private function _mouseUpDraggingEvent(event:MouseEvent)
		{
			// don't bubble up, which would trigger the mouse up on the stage
    		event.stopImmediatePropagation();
			//remove the listener of the stage after releasing the slider
			stage.removeEventListener(MouseEvent.MOUSE_UP, _releaseOutsideDragging);
			_drop(event);
		}
		...
		//	function _mouseUpDraggingEvent
		//================================
		//This function triggers from the listener of the stage as result of Mouse_UP event
		private function _releaseOutsideDragging(event:MouseEvent)
		{
			//remove the listener of the stage after releasing from outside the slider
			stage.removeEventListener(MouseEvent.MOUSE_UP, _releaseOutsideDragging);
			_drop(event);
		}
		//==========================
		...
		{
			Tracer.debugTrace("PlaybackSlider: _drop", 4);
            this._slider.removeEventListener(MouseEvent.MOUSE_MOVE, _updateUponDrag);
            this._slider.stopDrag();
			//update value
			_calcValue();
			
			///release _fDraggingPlaybackSlider
			_fDraggingPlaybackSlider = false;
			stage.removeEventListener(MouseEvent.MOUSE_MOVE, _updateIfMouseMoveOutOfSlider);
			//fire ebVBPlayAfterPauseForSlider event
			if( (_strVideoStatus=="Playing" || _strVideoStatus=="Loading") && this._value < 100 )
			{
				Tracer.debugTrace("PlaybackSlider: _drop: fire ebVBPlayAfterPauseForSlider event", 4);
				var ev:VideoEvent = new VideoEvent("ebVBPlayAfterPauseForSlider", "");
				dispatchEvent(ev);
			}
        }
		//==========================
		...
		//==========================
		//This function removes all dragging listeners (from stage and slider)
		...
		//============================
		//	function _updateIfMouseMoveOutOfSlider
		//============================
		///This function updates the volume upon mouse move while the slider is dragged AND user moves the mouse out of the slider rollOver rect
		private function _updateIfMouseMoveOutOfSlider(event:MouseEvent):void 
		{
			if(_fDraggingPlaybackSlider && (mouseY < 0 || mouseY > 9))
			{
				if(mouseX > this._trail.x && mouseX < _draggingAreaWidth)
				{
					//update value
		...
		private function _updateUponClick(event:MouseEvent):void 
		{
			...
			var ev:VideoEvent = new VideoEvent("ebVBSendSliderDraggedInteraction", "");
			dispatchEvent(ev);
			//calc new x pos
		...
		private function _setEvents():void
		{
			try
			{
				...
				//to the videoLoader
				this._eventsArr = new Array("ebVLPlayProgress", "ebVLLoadProgress", "ebVLStatusChanged", "ebUpdatePlaybackSlider");
				this._compEventsArr = new Array("ebVBUpdatePlayhead", "ebVBPauseForSlider", "ebVBPlayAfterPauseForSlider", "ebVBSendSliderDraggedInteraction");
				this.setEvents();
			}catch (error:Error)
			{
		...
		protected override function _setAttrFromLoader():void
		{
			try
			{
				...
				if(typeof(this._videoLoaderInst) != "undefined")
				{
					///Enable dragging for eny type of video (progressive and streaming)
					this._fEnableDragging = true; 
