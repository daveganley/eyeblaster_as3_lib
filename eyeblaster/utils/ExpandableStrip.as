//****************************************************************************
//class eyeblaster.utils.ExpandableStrip
//-------------------------------------------
// The ExpandableStrip component runs as a small video strip (banner) which,
...
package eyeblaster.utils
{
	...
	public class ExpandableStrip extends MovieClip
	{
		//++++++++++++++++++++++++++++++++++++++++++++++++++++
		//					 Attributes
		//++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		private var _nStripLoopLen:Number = 0;
		...
		public function set stripLoopLen(sec:Number):void
		{
			this._nStripLoopLen = sec;
		}
		public function get stripLoopLen():Number
		{
			return this._nStripLoopLen;
		}
		...
		private static var _nInstCount:Number = 0;	//ExpandableStrip component instance count
		...
		//	Constructor
		//===============
		public function ExpandableStrip()
		{  
			Tracer.debugTrace("ExpandableStrip: Constructor", 6);
			Tracer.debugTrace("ExpandableStrip version: "+compVersion, 0);
			Tracer.debugTrace("Eyeblaster Workshop | ExpandableStrip | You are currently using a deprecated version of the component, please import a newer version to stage.  For more information see the on-line help.",0);
		...
		{
			Tracer.debugTrace("ExpandableStrip: initUponEnterFrame", 6);
		...
		//The event is fired after iniliazing all the attributes of the video component, otherwise parameters like _nStripLoopLen
		...
		//There are parameters that are transferred from the ExpandableStrip to the VideoLoader.
		...
		{
			Tracer.debugTrace("ExpandableStrip: setVideoComponent- " + videoMC, 1);
			...
			{
				Tracer.debugTrace("ExpandableStrip: setVideoComponent Error: "+ err.message, 1);
			}
		}
		...
		{
			Tracer.debugTrace("ExpandableStrip: expand", 4);
			try
			{
			...
			{
				Tracer.debugTrace("ExpandableStrip: expand Error: "+ err.message, 1);
			}
		}
		...
		{
			Tracer.debugTrace("ExpandableStrip: retract", 3);
			try
			{
			...
			{
				Tracer.debugTrace("ExpandableStrip: retract Error: "+ err.message, 1);
			}
		}
		...
		{
			Tracer.debugTrace("ExpandableStrip: _init", 4);
			try
			{
				this._initComp();
				this._initAttr();
		...
		//	function _initAttr
		//========================
		//The function inits all the attributes of the ExpandableStrip class
		private function _initAttr():void
		{
			Tracer.debugTrace("ExpandableStrip: _initAttr", 4);
			this._compBounds = this.getBounds(root);			//ExpandableStrip component boundaries
		...
		{
			Tracer.debugTrace("ExpandableStrip: _initComp", 4);
			EB.ebSetComponentName("VideoStrip");
		...
		{
			Tracer.debugTrace("ExpandableStrip: _tweenExpand", 4);
		...
		{
			Tracer.debugTrace("ExpandableStrip: _tweenRetract", 4);
			//resize the component 
		...
		{
			Tracer.debugTrace("ExpandableStrip: _convertEasingToFunction", 4);
		...
		{
			Tracer.debugTrace("ExpandableStrip: _mouseMovve", 4);
			try
			{
			...
			{
				Tracer.debugTrace("ExpandableStrip: _mouseMove Error: "+ err.message, 1);
			}
		}
		...
		{
			Tracer.debugTrace("ExpandableStrip: _showHideElements (" + fShow + ") _fElementsAreHidden=" + _fElementsAreHidden, 1);
		...
		function _calcPosAndSize()
		{
			Tracer.debugTrace("ExpandableStrip: _calcPosAndSize", 6);
		...
		private function _videoCompUponRetract()
		{
			...
			this._videoComp.setVideoLoop(this._nStripLoopLen);
		...
		{
			Tracer.debugTrace("ExpandableStrip: _mouseOut", 4);
		...
		{
			Tracer.debugTrace("ExpandableStrip: _setVideoUponEnterFrame", 6);
		...
		{
			Tracer.debugTrace("ExpandableStrip: setVideoComponent- " + this._videoComp, 1);
			try
			{
				...
				if (this._videoComp == null) //_videoComp is null if compInst is "" and the API function setVideoComponent was not triggered or in the UI we gave a name that its instance doesn't exist
					Tracer.debugTrace("Eyeblaster ExpandableStrip component | The instance name you entered does not exist", 0);
				else
				{
					...
					this._videoComp.setVideoLoop(_nStripLoopLen);	//set the default loop (full movie).
