//****************************************************************************
//class eyeblaster.targeting.CreativeState
//------------------------------------
...
package eyeblaster.targeting
{
	...
	public class CreativeState extends MovieClip
	{
		...
		private var _lvl:String = "Campaign";  //the state level. can be campaign or ad. by default campaign;
		...
		//sets the state level to be the campaign or the ad (according to the given parameter).
		//Parameters:
		// "Ad" - setting the state level to the ad.
		// "Campaign" - setting the state level to the campaign.
		public function set level(lvl) 
		{
			var urlParams = EBBase.urlParams;
			if (lvl.toLowerCase() == "ad")
			{
				_soName = urlParams.ebAdID;
				_lvl = "Ad";
			}
			else if (lvl.toLowerCase() == "campaign")
			{
				_soName = urlParams.ebCampaignID;
				_lvl = "Campaign";
			}
		}
		//get teh state level
		public function get level():String{return _lvl};
		...
		private function trimStatesList(str) 
		{
		...
		public function registerAdvancedState(stateNames) 
		{
			try
			{
				...
				//remove states that contains character '_' - limitation of the server
				var arrStates:Array = stateNames.split(",");
				for (var i=0; i<arrStates.length; i++)
				{
					if((arrStates[i]).indexOf("_") != -1)
					{
						Tracer.debugTrace("CreativeState: The state " +  arrStates[i] + "contains an illegal character '_' therefore it will not be included in the state list.", 0);
						arrStates.splice(i,1);
						i--;
					}
				}
				stateNames = arrStates.join(",");
		...
		// 	only one state for the campaign level, and when refering to
		// 	"state name" it is actually the state value
		public function setAdvancedState(name) 
		{
			try
			{
				//remove all starting and ending spaces.
				...
				EBBase.handleCommand("ebSetState", name);
			}catch(err:Error)
			{
		...
		// 	only one state for the campaign level, and when refering to
		// 	"state name" it is actually the state value
		public function resetAdvancedState() 
		{
			Tracer.debugTrace("CreativeState: resetState", 6);
			try
			{
				EBBase.handleCommand("ebResetState");
			}catch(err:Error)
			{
		...
		private function _initComp():void
		{
			...
			EBBase.ebSetComponentName("CreativeState");
